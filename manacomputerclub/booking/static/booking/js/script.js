...
// 
// }));
// const dataElement = document.getElementById('submit').textContent;
...
function updateSelectedComputers() {
  ...
  const selectedComputers = Array.from(
    document.querySelectorAll(".card.selected")
  ).map((card) => card.textContent);
  selectedComputersDiv.innerHTML =
    selectedComputers.length > 0
      ? "<strong>Выбранные компьютеры:</strong><br>" +
        selectedComputers
          .map((computer) => "<li>" + computer + "</li>")
          .join("")
      : "";
}
...
document
  .querySelectorAll('input[name="duration"], select[name="service"]')
  .forEach((input) => {
